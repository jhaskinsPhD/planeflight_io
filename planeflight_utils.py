...
def _check_str_arr_inputs(item, item_nm: str, allowed_vals, allowed_nm: str,
    ...
    if isinstance(item, str) and wildcard_str is True:
        if debug:
        ...
        if item.lower() != '?all?':
            ...
            give = item
            if debug:
                print("Item is string and not allowed wildcard.")
        else:
            if debug:
                print("Item is string and is an allowed wildcard.")
            is_error = False
            err_reason = ''
            give = allowed_vals

    # If input is array like... recognizes lists, np.ndarrays, pd.Series, or xr.dataarrays
    elif ((isinstance(item, list)) or (isinstance(item, np.ndarray)) or (isinstance(item, pd.core.series.Series))
          or (isinstance(item, xr.core.dataarray.DataArray))):
        if debug:
        ...
        if not all(isinstance(elem, (str, np.str_)) for elem in item):
            ...
            give = item
            if debug:
                print('Item is iterable, but not all elements are str.')
        else:
            if debug:
                print('Item is ierable and all are string. ')
            # Check that all elements listed in item are indeed in allowed_vals:
            if any(elem not in allowed_vals for elem in item):
                if debug:
                    print('Item is iterable of strs but has some unallowed values.')
                ...
                if debug:
                    print('Item is ierable, all are string, and all allowed. ')
                is_error = False
                err_reason = ''
                give = item
    else:
        ...
        if debug:
            print('Item was not identified and vals assigned were unknown.')
        is_error = True
...
def _read_planelog_to_df(filename: str):
    ...
    for i in range(0, len(lines)):
        ...
        if i == 2:
            split_lines = True if odd_len != even_len else False
